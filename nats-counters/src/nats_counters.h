 ...
 *
 *  Value: any valid signed integer string, e.g. "+5" or "-3".
 */
 ...
 * @param value last-seen counter value for that source (string, arbitrary precision).
 * @param closure the user-supplied pointer.
 */
typedef void (*natsCounterSourceIterFn)(const char *stream,
                                        const char *subject,
                                        const char *value,
                                        void       *closure);
 ...
 */
void natsCounter_Destroy(natsCounter *counter);
 ...
 *
 * This is the C equivalent of Go's `AddInt(ctx, subject, value) (*big.Int, error)`.
 * The delta is a convenient `long long`, but the returned value will never
 * overflow because it is a heap-allocated decimal string.
 *
 * @param counter the counter.
 * @param subject the subject to modify.
 * @param delta amount to add (may be negative).
 ...
 */
bool natsCounterEntry_HasSources(natsCounterEntry *entry);
